...
class MMR_pipeline_:
    ...
    def __init__(self,
        ...
        # 从配置cfg.TRAIN.MMR.layers_to_extract_from中读取要提取的层名称到extract_layer。
        for extract_layer in cfg.TRAIN.MMR.layers_to_extract_from:
            # 定义一个ForwardHook类,用于提取层的输出
            forward_hook = ForwardHook(self.teacher_outputs_dict, extract_layer)
            # 从当前模型cur_model中获取extract_layer对应的具体层network_layer
            network_layer = cur_model.__dict__["_modules"][extract_layer]
            #  在network_layer的最后一层([-1])注册前向钩子forward_hook
