...
# 损失函数定义
# CosineSimilarity计算两个高维特征图(B,C,H,W)中各个像素位置的特征相似度，即特征图中的每个像素位置上的一个(B,C,1,1)的向量为该位置的特征，总共有BxHxW个特征
